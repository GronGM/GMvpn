...
from vpn_client.dataplane import LinuxUserspaceDataPlane, NullDataPlane
...
def main() -> int:
    ...
    network_stack = (
        LinuxNetworkStack(dry_run=not args.apply_network_changes)
        if args.platform == "linux"
        else SimulatedNetworkStack()
    )
    dataplane = (
        LinuxUserspaceDataPlane(dry_run=not args.apply_network_changes, state_store=backend_state_store)
    ...
    if isinstance(network_stack, LinuxNetworkStack) and network_stack.last_plan:
        ...
        if network_stack.last_plan.rollback_commands:
            ...
            for command in network_stack.last_plan.rollback_commands:
                print(f"  - {' '.join(command)}")
    if isinstance(network_stack, LinuxNetworkStack) and network_stack.last_reconciliation:
